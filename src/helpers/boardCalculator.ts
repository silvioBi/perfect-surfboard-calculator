...
const getSurferWeightAs5Multiple = (surfer: Surfer): number => {
    ...
    if (weightAs5Multiple < 35)
        return 35
    if (weightAs5Multiple  > 110)
        return 110
...
const getIdealSurfboardVolumeInLiters = (surfer: Surfer): number => {
    ...
    const baseVolume = WEIGHT_VOLUME_LITERS_TABLE[surfer.skillLevel][weightAs5Multiple.toString()]

    let ageFactor = 1;
    Object.entries(AGE_VOLUME_FACTOR).forEach(([ageLimit, factor]) => {
        if (surfer.age < ageLimit)
            ageFactor = factor
    })
    let fitnessFactor = FITNESS_VOLUME_FACTOR[surfer.fitnessLevel]
    return parseFloat((baseVolume * ageFactor * fitnessFactor).toFixed(2));

}
const getIdealSurfboardShapesForSurfer = (surfer: Surfer, waveHeight: number): ShapeType[] => {
    let shapesForWaveHeight = SHAPE_TYPE_BASED_ON_WAVE_HEIGHT_CM['30'][surfer.skillLevel]; // default
    Object.entries(SHAPE_TYPE_BASED_ON_WAVE_HEIGHT_CM).forEach(([waveHeightForShape, shapes]) => {
...
const getIdealSurfboardLengthHumanReadable = (surfer: Surfer, shape: ShapeType): string => {
    const weightAs5Multiple = getSurferWeightAs5Multiple(surfer)
    const ranges = SHAPE_LEGTH_FEET_BASED_ON_SURFER_WEIGHT[shape][surfer.skillLevel]
    let rangeForWeight = ranges['65']
    Object.entries(ranges).forEach(([weightRange, range]) => {
        if (parseInt(weightRange) <= weightAs5Multiple)
            rangeForWeight = range
    })
    let rmin = rangeForWeight?.min, rmax = rangeForWeight?.max;
    if (rmin && !rmax)
