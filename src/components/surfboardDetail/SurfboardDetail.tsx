...
import {
  ...
  getSurfboardImage,
} from "../../helpers/surfboardInfo";

interface Props {
  surfboard: Surfboard;
}

const renderProperty = (name: string, value: React.ReactNode) => {
  return (
    <HStack>
...
const SurfboardDetail = ({ surfboard }: Props) => {
  return (
    <Box maxW="xs" borderWidth="1px" borderRadius="lg" overflow="hidden">
      ...
      />
      <Box m="2" fontWeight="semibold" as="h4" noOfLines={1}>
        {surfboard.shapeType[0].toUpperCase() +
          surfboard.shapeType.slice(1).toLowerCase()}
      </Box>
      <Box m="2">
